...
function EditCardPage() {
    ...
    useEffect(() => {
        ...
        setSubTitle(card?.subtitle as string)
        setDescription(card?.description as string)
        setPhone(card?.phone as string)
        setEmail(card?.email as string)
        setWebsite(card?.website as string)
        setImageUrl(card?.imageUrl as string)
        setImageAlt(card?.imageAlt as string)
        setState(card?.state as string)
        setCountry(card?.country as string)
        setCity(card?.city as string)
        setStreet(card?.street as string)
        setHouseNumber(card?.houseNumber as string)
        setZipCode(card?.zipCode as string)
    }, [card])
    ...
    return (
        <>
            <FormHeader title="EDIT CARD" />
            <Typography variant="h6" sx={{ textAlign: 'center' }}>Card no. {card?._id}</Typography>

            <FormLayout onReset={resetForm} onSubmit={onEdit} >
                <InputContainer>
